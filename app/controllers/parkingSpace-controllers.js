...
parkingSpaceCntrl.register = async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      parkingSpace.image = req.file.filename; // save only filename in DB
    }

    // Set owner ID from authenticated user
    parkingSpace.ownerId = req.user.id;

    // Use geoapify API to get coordinates
    if (parkingSpace.address && parkingSpace.address.area) {
      const response = await axios.get(
        `https://api.geoapify.com/v1/geocode/search?text=${parkingSpace.address.area}&apiKey=4a35345ee9054b188d775bb6cef27b7c`
      );
      ...
      if (
        ...
        parkingSpace.address.coordinates = reverseLatLon(
...
parkingSpaceCntrl.update = async (req, res) => {
  ...
  try {
    ...
    spaceBody.address.coordinates = reverseLatLon(
      response.data.features[0].geometry.coordinates
    );
